...
- 间距操作直接传入滑块偏移，通过 CFPreferences 写入当前用户、当前主机的全局设置并校验；偏移为零时删除覆盖值。界面在写入成功后更新已保存值，避免依赖 Combine 异步同步设置的顺序。
- macOS 26 的间距应用只刷新控制中心；较早系统尝试正常重启相关应用，退出等待有时限且不强退第三方应用。实机已验证应用 `-6` 后系统图标区域缩小，以及重置会删除两项全局覆盖值。
- 间距的应用和重置使用原生 NSButton，并复用同一个 SwiftUI 包装组件。旧 SwiftUI 按钮出现自动化能触发、用户手动点击却未进入保存流程的情况；原生版本已由用户连续手动验证多个间距值和重置，日志记录完整的鼠标按下、按钮操作与写入过程。
