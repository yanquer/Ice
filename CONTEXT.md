...
- 图标回藏后可能仍有窗口与布局宽度，但单窗口和合成截图都完全透明。截图缓存需逐项检查 Alpha，空图不得覆盖最后有效快照；合成截图中的失败项目单独重试，回藏前主动保存可见图标的快照。没有有效快照时保留可点击的恢复入口。
- Tahoe 的窗口 `isOnScreen` 可能在实际坐标已移到屏幕外时仍为 true，临时显示入口需同时检查窗口矩形位于显示器内，避免直接向屏幕外发送点击。
